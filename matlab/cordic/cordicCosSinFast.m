function [ cosCrd, sinCrd ] = cordicCosSinFast( phi, N )
% function [ cosCrd, sinCrd ] = cordicCosSinFast( phi, N )
%
% calculate cosine and sine by CORDIC algorithm
...
L = length( phi );
cosCrd = int64( zeros( 1, L ) );
sinCrd = int64( zeros( 1, L ) );
% CORDIC algorithm
parfor i = 1 : L
    x = coefd;
    y = int64( 0 );
...
cosCrd( qrt ) = - cosCrd( qrt );

% limit outputs
cosCrd( cosCrd >   2 ^ ( PHI_WDT - 2 ) ) =   2 ^ ( PHI_WDT - 2 );
cosCrd( cosCrd < - 2 ^ ( PHI_WDT - 2 ) ) = - 2 ^ ( PHI_WDT - 2 );
sinCrd( sinCrd >   2 ^ ( PHI_WDT - 2 ) ) =   2 ^ ( PHI_WDT - 2 );
sinCrd( sinCrd < - 2 ^ ( PHI_WDT - 2 ) ) = - 2 ^ ( PHI_WDT - 2 );

% cast int64 to fi
T = numerictype( 1, PHI_WDT, PHI_WDT - 2 );
cosCrd = fi( double( cosCrd ) / 2 ^ ( PHI_WDT - 2 ), T );
sinCrd = fi( double( sinCrd ) / 2 ^ ( PHI_WDT - 2 ), T );